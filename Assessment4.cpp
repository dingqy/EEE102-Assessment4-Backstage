...
int main() {
	...
	Book temp2 = Book("Algorithms", "Wayne", "Princeton", "Stored", "0", 40.0, 1);
	temp.addBook(temp2);
	temp2 = Book("Computer Systems12", "Patt", "UTAustin", "Stored", "0", 40.0, 2);
	temp.addBook(temp2);
	...
	cout << temp.getFine(temp4[0]) << endl;
	cout << temp.bookReverse(temp2, stu1);
	temp.returnBook(temp2);
	if (!temp.borrowBook(temp2, user1)) {
		cout << "Can not borrow" << endl;
	}
	temp.borrowBook(temp2, stu1);
	temp.exit();
	/*sqlite3* db;
